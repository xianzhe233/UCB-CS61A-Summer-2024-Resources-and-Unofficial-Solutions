...
class ThrowerAnt(Ant):
    ...
    food_cost = 3
    lower_bound = 0
    upper_bound = float('inf') # range = [lower_bound, upper_bound]
    ...
    def nearest_bee(self):
        ...
        scan_idx = 0
        while not scan_place.is_hive and scan_idx < self.lower_bound:
            scan_idx += 1
            scan_place = scan_place.entrance
        while not scan_place.is_hive and scan_idx <= self.upper_bound:
            scan_idx += 1
            if len(scan_place.bees):
...
class QueenAnt(ThrowerAnt):
    ...
    def double_damage(self):
        scan_place = self.place.exit
        while scan_place is not None:
            if scan_place.ant is not None:
                if scan_place.ant.doubled == False:
                    scan_place.ant.damage *= 2
                    scan_place.ant.doubled = True
                if scan_place.ant.is_container and scan_place.ant.ant_contained is not None:
                    if scan_place.ant.ant_contained.doubled == False:
                        scan_place.ant.ant_contained.damage *= 2
                        scan_place.ant.ant_contained.doubled = True
            scan_place = scan_place.exit
    ...
    def throw_at(self, target):
        ...
        def slow_action(gamestate):
            ...
            if gamestate.time % 2 == 0:
                ...
                pass
            # print("DEBUG:", gamestate.time, target.slow_count_down, target.place.name)
            if target.slow_count_down <= 0: # if not use <= here, it will make mistake.I don't know exactly why.
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        dist = 0
        insect_dict = {}
        while not scan_place.is_hive:
            if scan_place.ant is not self and scan_place.ant is not None:
                insect_dict[scan_place.ant] = dist
            for bee in scan_place.bees:
    ...
    def calculate_damage(self, distance):
        ...
        res = 2 - damage_decay * self.insects_shot - dist_decay * distance
        if res <= 0:
            return 0
        else:
            return res
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if self.scared and self.scare_count_down:
            self.scare_count_down -= 1
