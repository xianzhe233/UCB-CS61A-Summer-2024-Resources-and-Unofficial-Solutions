...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    dices = list(dice() for _ in range(num_rolls))
    if 1 in dices:
        return 1
    else:
        return sum(dices)
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    res_0_0 = strategy(0, 0)
    for i in range(goal):
        for j in range(goal):
            if strategy(i, j) != res_0_0:
                return False
    return True
    # END PROBLEM 7
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    return 0 if sus_update(0, score, opponent_score) >= threshold + score else num_rolls
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if sus_update(0, score, opponent_score) >= GOAL:
        return 0
    for i in (1, 2):
        if sus_update(0, int(score + ex[i]), opponent_score) >= GOAL:
            return i
    return 0 if sus_update(0, score, opponent_score) > sus_update(0, int(score + ex[6]), opponent_score) else 6
    # END PROBLEM 12
